...
describe('Components => rules/MinRule', function () {
  it('Should be useful with NUMBERS, by validating its VALUE', function () {
    ...
    expect(rule.withValue(2).isValid(3)).to.be.equals(true)
    ...
    expect(rule.withValue(2).isValid(1)).to.be.equals(false)
  })
  ...
  it('Should be useful with STRINGS, by validating its LENGTH', function () {
    ...
    expect(rule.withValue(10).isValid('#'.repeat(11))).to.be.equals(true)
    ...
    expect(rule.withValue(10).isValid('#'.repeat(9))).to.be.equals(false)
  })
})
