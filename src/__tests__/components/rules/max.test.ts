...
describe('Components => rules/MaxRule', function () {
  it('Should be useful with NUMBERS, by validating its VALUE', function () {
    ...
    expect(rule.withReference(2).isValid(1)).to.be.equals(true)
    // 2 is the maximum value, so 1 should not pass
    expect(rule.withReference(2).isValid(3)).to.be.equals(false)
  })
  ...
  it('Should be useful with STRINGS, by validating its LENGTH', function () {
    ...
    expect(rule.withReference(10).isValid('#'.repeat(9))).to.be.equals(true)
    ...
    expect(rule.withReference(10).isValid('#'.repeat(11))).to.be.equals(false)
  })
})
