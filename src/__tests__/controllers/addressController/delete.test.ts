...
function makeRequest () {
  return request?.delete(`/addresses/${validData.id}`)
...
describe.only('AddressController => delete', function () {
  ...
  it('Should delete and return the deleted address', async function () {
    sandbox.stub(AddressRepository.prototype, 'find').resolves(addressDto)
    const spy = sandbox.stub(AddressRepository.prototype, 'delete').resolves(addressDto)
